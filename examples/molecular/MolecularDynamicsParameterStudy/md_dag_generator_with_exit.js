...
function createWf(functionName, molecules, end_time, temperatureArray) {
  ...
  for (i=0; i<temperatureArray.length; i++) {
    ...
    wfOut.processes.push(
      task("make-movie-" + temperature, functionName, "./make-movie.sh", [input, output], [input], [output, "end"])
    );
  }
  
  wfOut.processes.push(
      task("kill-hflow", functionName, "./kill-hflow.sh", [], ["end:" + temperatureArray.length], [])
  );
